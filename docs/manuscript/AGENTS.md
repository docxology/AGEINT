...
- Do not add numbered chapter Markdown files under `docs/manuscript/`.
- Full config surface: see [`config.yaml.example`](config.yaml.example); live values in [`config.yaml`](config.yaml).
- Generated semantic prose lives only under `output/manuscript/` (not under `docs/manuscript/`). Never hand-edit generated markdown; fix templates, `src/manuscript_manifest/`, or `src/figures/`, then rebuild.
