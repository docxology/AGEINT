...
def draw_title_band(draw: Any, font_mod: Any, font_fn: Any, title: str, *, subtitle: str = "", width: int = 1600, height: int = 118, accent: str = "#2563eb") -> None:
...
def draw_footer(draw: Any, font_mod: Any, font_fn: Any, text: str, *, width: int = 1600, y: int = 930) -> None:
...
def draw_wrapped_text(draw: Any, xy: tuple[float, float], text: str, font: Any, *, fill: str = INK, width: int = 28, max_lines: int = 3, line_height: int = 25) -> None:
    x, y = xy
    for index, line in enumerate(wrap_lines(text, width, max_lines)):
        draw.text((x, y + index * line_height), line, fill=fill, font=font)


def draw_centered_text(draw: Any, box: tuple[float, float, float, float], text: str, font: Any, *, fill: str = INK, width: int = 20, max_lines: int = 3, line_height: int = 27) -> None:
    x0, y0, x1, y1 = box
...
def draw_arrow(draw: Any, start: tuple[float, float], end: tuple[float, float], *, fill: str = "#334155", width: int = 5) -> None:
    sx, sy = start
    ex, ey = end
