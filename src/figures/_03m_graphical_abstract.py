...
def _draw_background(draw: Any) -> None:
    ...
    for radius, color in ((960, "#dbeafe"), (760, "#dcfce7"), (560, "#fef3c7")):
        draw.ellipse((1200 - radius, 1150 - radius, 1200 + radius, 1150 + radius), outline=color, width=10)
...
def _draw_header(draw: Any, font_mod: Any, title: str) -> None:
    ...
    draw.text((248, 226), "Evidence-bounded curriculum architecture for Synthetic Analytic Tradecraft; not a capability benchmark", fill=MUTED, font=_font(font_mod, 27))
...
def _draw_agent_layer(draw: Any, font_mod: Any) -> None:
    ...
    steps = [("retrieve", "direct sources"), ("reason", "alternatives"), ("tool", "allowlist"), ("memory", "governed"), ("handoff", "human gate")]
...
def _draw_discipline_lanes(draw: Any, font_mod: Any) -> None:
    ...
    for index, (box, title, body) in enumerate(lanes):
        ...
        draw_wrapped_text(draw, (box[0] + 28, box[1] + 65), body, _font(font_mod, 22), fill=MUTED, width=27, max_lines=2, line_height=27)
...
def _draw_tradecraft_core(draw: Any, font_mod: Any) -> None:
    ...
    draw_wrapped_text(
        draw,
        (805, 985),
        ("Evidence-bounded records and public sources become claim packets: observation, inference, assumption, likelihood, confidence, dissent, accountability, caveat, owner, and refresh trigger."),
        _font(font_mod, 29),
        fill=MUTED,
        width=52,
        max_lines=4,
    ...
    fields = ["observation", "inference", "assumption", "likelihood", "confidence", "dissent", "accountability", "refresh"]
    for index, field in enumerate(fields):
        row = index // 4
        col = index % 4
        x0 = 780 + col * 225
        y0 = 1175 + row * 70
        draw.rounded_rectangle((x0, y0, x0 + 190, y0 + 48), radius=18, fill=SOFT_PALETTE[index % len(SOFT_PALETTE)], outline="#94a3b8", width=2)
...
def _draw_human_gate(draw: Any, font_mod: Any) -> None:
    ...
    draw.text((845, 1588), "approve for reviewable product, assign revision, or stop and rebuild", fill="#78350f", font=_font(font_mod, 24))
...
def _draw_verification_ring(draw: Any, font_mod: Any) -> None:
    ...
    for index, (box, title, body) in enumerate(checks):
        ...
        draw_wrapped_text(draw, (box[0] + 28, box[1] + 60), body, _font(font_mod, 20), fill=MUTED, width=27, max_lines=3, line_height=24)
...
def _draw_source_spine(draw: Any, font_mod: Any) -> None:
    ...
    draw.text((275, 1995), "locked source identities, verified anchors, local figure registry, citation inventory, and rebuildable manuscript outputs", fill="#cbd5e1", font=_font(font_mod, 24))
    tiles = [
...
def _draw_footer(draw: Any, font_mod: Any) -> None:
    ...
    draw.text((220, 2238), "Reader contract: every visible claim must trace to a source, an artifact, a verifier, a review decision, and a refresh path.", fill="#334155", font=_font(font_mod, 25))


def _draw_decision_card(draw: Any, font_mod: Any, box: tuple[int, int, int, int], title: str, body: str, color: str) -> None:
    ...
    draw_wrapped_text(draw, (box[0] + 30, box[1] + 74), body, _font(font_mod, 21), fill=MUTED, width=27, max_lines=3, line_height=25)


def _arrow(draw: Any, start: tuple[int, int], end: tuple[int, int], color: str, *, width: int = 8) -> None:
    sx, sy = start
    ex, ey = end
