...
from rendered_reference_audit import sanitize_rendered_section_title_mentions, section_title_rules
from _markdown_split import DEFAULT_MAX_TEXT_FILE_LINES, split_by_line_budget as _split_by_line_budget, split_h2_blocks as _split_h2_blocks

from .types import ManuscriptSection, ordering_config_yaml as _ordering_config_yaml, slugify as _slug
...
def _split_assurance_fragments(section: ManuscriptSection, assurance_block: str) -> tuple[str, str, str]:
    ...
    evidence_block, governance_and_assessment = _split_before_h3(assurance_block, scaffolds["governance"])
    governance_block, assessment_block = _split_before_h3(governance_and_assessment, scaffolds["assessment"])
...
def _chapter_fragments(section: ManuscriptSection, rendered: str) -> list[tuple[str, str]]:
    ...
    headings = chapter_landmark_titles(section.title, profile_title=section.context.get("CHAPTER_PROFILE_TITLE", ""), practice_lens_title=section.context.get("CHAPTER_PRACTICE_LENS_TITLE", ""))
    assurance_block = block_map.get(headings["assurance"], "")
    evidence_block, governance_block, assessment_block = _split_assurance_fragments(section, assurance_block)
    path = Path(section.relative_path)
    base_dir = path.parent / path.stem
    grouped = [
        ("00-overview.md", [lead, block_map.get(headings["frame"], "")]),
    ...
    wayfinding_footer = f"**Where this sits.** {lead} the curriculum atlas [@sec:curriculum_orientation] for the reader paths, evidence map, and safety gates that govern this module."
    wayfinding_fragments = {"02-evidence-contract.md", "03-governance-boundary.md"}
    fragments: list[tuple[str, str]] = []
    for file_name, parts in grouped:
        text = "\n\n".join(part for part in parts if part).rstrip()
        if text:
            for frag_path, fragment in _split_by_line_budget((base_dir / file_name).as_posix(), text):
...
def _front_or_appendix_fragments(section: ManuscriptSection, rendered: str, base_dir: str) -> list[tuple[str, str]]:
    lead, blocks = _split_h2_blocks(rendered)
    fragments: list[tuple[str, str]] = []
...
def render_manuscript(project_root: Path, curriculum: Curriculum, variables: dict[str, str], figure_registry: list[dict[str, Any]] | None = None) -> Path:
    ...
    rendered_units = [{"id": unit["id"], "directory": unit["directory"], "chapters": []} for unit in manifest.units]
    ...
    for section in manifest.sections:
        ...
        visual_synthesis = _visual_synthesis(root, out_dir, section, manifest, figures, render_relative_path=_first_fragment_path(section))
        early_visual_context = orientation_early_visual_context(root, out_dir, section, figures, render_relative_path=_first_fragment_path(section))
        context = {**variables, **section.context, **early_visual_context, "VISUAL_SYNTHESIS": visual_synthesis}
        ...
        if section.kind == "front":
            ...
            rendered_unit["chapters"].extend(Path(relative_path).relative_to(unit_dir).as_posix() for relative_path, _ in fragments)
