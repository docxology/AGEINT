...
from ._01_part import CoursebookProfile, IntelligenceProfile, PracticeLens, ResearchAnchor, SafePatternProfile
...
def build_coursebook_profiles() -> dict[str, CoursebookProfile]:
    ...
    return {str(row["identifier"]): CoursebookProfile(**row) for row in coursebook_profiles_payload()}
...
def profile_for_titles(part_title: str, section_title: str = "", chapter: dict[str, object] | None = None) -> IntelligenceProfile:
...
def _cached_practice_lens_for_titles(part_title: str, section_title: str) -> PracticeLens:
    ...
    if any(term in part_key for term in ("industrial", "ics", "operational technology")) and any(term in section_key for term in ("incident", "attack", "att ck", "threat")):
...
def practice_lens_for_titles(part_title: str, section_title: str = "", chapter: dict[str, object] | None = None) -> PracticeLens:
...
def safe_pattern_rows(patterns: list[dict[str, Any]]) -> str:
    ...
    rows = ["| Source identity | Safe curriculum treatment | Methods | Defensive application | Safety boundary |", "|---|---|---|---|---|"]
    for pattern in patterns:
        number = int(pattern["number"])
        profile = _safe_pattern_profile(number)
        rows.append(f"| Pattern {number}: {pattern['name']} (source identity only) | {profile.safe_name} | {profile.methods} | {profile.application} | {profile.safety_boundary} |")
    return "\n".join(rows)


def safe_pattern_treatment(section_title: str, active_pattern_number: int | None = None) -> tuple[str, int | None]:
    ...
    if pattern_match:
        ...
        profile = _safe_pattern_profile(number)
        return (f"Pattern {number}: {profile.safe_name} (source identity preserved in pattern registry) - {profile.safety_boundary}", number)
    ...
    if section_title.startswith("Code Archetype:"):
        return (f"Safe architecture artifact: diagram an allowlisted, logged, revocable workflow for {profile.safe_name}", active_pattern_number)
