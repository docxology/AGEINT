...
from build_clock import SOURCE_DATE_EPOCH_ENV, build_date, build_datetime, build_timestamp
...
def test_pinned_epoch_makes_repeated_reads_identical(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_blank_epoch_falls_back_to_the_real_clock(monkeypatch: pytest.MonkeyPatch, blank: str) -> None:
...
def test_malformed_epoch_raises_rather_than_silently_drifting(monkeypatch: pytest.MonkeyPatch, bad: str) -> None:
...
def test_build_date_is_the_utc_date_of_the_instant(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(SOURCE_DATE_EPOCH_ENV, str(PINNED_EPOCH))
    observed = build_date()
