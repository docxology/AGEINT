...
from analysis_validation import ANALYSIS_VALIDATION_LANES, analysis_validation_family_rows, analysis_validation_matrix_rows
...
def test_visual_quality_audit_matches_rendered_registry() -> None:
    ...
    assert audit["summary"]["kind_counts"] == {FigureKind.AI_GENERATED.value: 6, FigureKind.HISTORICAL.value: 4, FigureKind.MERMAID.value: 115, FigureKind.PYTHON.value: 52}
...
def test_visual_quality_dashboard_is_registry_backed() -> None:
    ...
    dashboard = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-visual-quality-audit-dashboard")
...
def test_artifact_evidence_control_loop_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-artifact-evidence-control-loop")
...
def test_scholarship_triangulation_map_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-scholarship-triangulation-map")
    assert figure["kind"] == FigureKind.PYTHON.value
...
def test_synthetic_tradecraft_method_contract_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-synthetic-tradecraft-method-contract")
    assert figure["kind"] == FigureKind.PYTHON.value
...
def test_analysis_validation_matrix_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-analysis-validation-matrix")
    assert figure["kind"] == FigureKind.PYTHON.value
    assert figure["provenance"]["renderer_id"] == "analysis_validation_matrix"
    assert "claims reviewable by class" in figure["caption"]
    assert [row[0] for row in analysis_validation_matrix_rows()] == [lane.claim_class for lane in ANALYSIS_VALIDATION_LANES]
...
def test_analysis_validation_family_coverage_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-analysis-validation-family-coverage")
    assert figure["kind"] == FigureKind.PYTHON.value
...
def test_source_metadata_integrity_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-source-metadata-integrity")
    assert figure["kind"] == FigureKind.PYTHON.value
...
def test_source_refresh_due_dashboard_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-source-refresh-due-dashboard")
    assert figure["kind"] == FigureKind.PYTHON.value
...
def test_agency_source_coverage_dashboard_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-agency-source-coverage")
    assert figure["kind"] == FigureKind.PYTHON.value
...
def test_claim_calibration_visual_is_registry_backed() -> None:
    registry = load_figure_registry(PROJECT_ROOT / "output/figures/figure_registry.json")
    figure = next(entry for entry in registry["figures"] if entry["label"] == "fig:ageint-claim-calibration-and-visual-semantics")
    assert figure["kind"] == FigureKind.PYTHON.value
