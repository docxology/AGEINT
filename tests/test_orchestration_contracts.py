...
from intelligence_content.source_packs import source_pack_contract_report, source_pack_registry
from orchestration_audit import collect_orchestration_contract, render_orchestration_contract_markdown
from orchestration_contracts import output_build_sentinels, pipeline_stage_contracts, source_freshness_roots, validate_pipeline_stage_contracts
...
def test_pipeline_stage_registry_is_complete_acyclic_and_derives_freshness_contracts() -> None:
    ...
    assert {"source_validation", "curriculum_build", "variables_and_bibliography", "figures", "manuscript_render", "evidence_transit", "artifact_reports"} <= stage_ids
...
def test_source_pack_registry_validates_known_keys_and_source_class_errors(tmp_path: Path) -> None:
    ...
    data_dir.mkdir()
    (data_dir / "agency_source_packs.yaml").write_text("packs:\n  agency_pack:\n    - official_known\n", encoding="utf-8")
    (data_dir / "research_source_packs.yaml").write_text("packs:\n  research_pack:\n    - scholarly_missing\nprofile_routes:\n  analytic_tradecraft:\n    - research_pack\n", encoding="utf-8")
