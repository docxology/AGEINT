...
from published_counts_gate import (
    compare_counts,
    recompute_figure_count,
...
def test_recompute_source_counts_matches_published_stats() -> None:
    ...
    outline = json.loads(
        (PROJECT_ROOT / "output" / "data" / "curriculum_outline.json").read_text(encoding="utf-8")
    )
...
def test_verify_published_counts_passes_on_real_repo_data() -> None:
    report = verify_published_counts(
        PROJECT_ROOT,
        figure_count=recompute_figure_count(
...
def test_compare_counts_flags_every_drift_axis() -> None:
    ...
    mismatches = compare_counts(
        recomputed,
        published,
        figure_count_recomputed=177,
        figure_count_published=176,
    )
...
def test_verify_published_counts_raises_on_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="figures"):
        verify_published_counts(
            PROJECT_ROOT,
            curriculum_stats=recomputed,
            figure_count=recomputed["parts"],
        )
