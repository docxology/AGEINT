...
def test_related_work_protocol_and_memory_boundaries_render(built_output: Path) -> None:
    ...
    assert "not a new agent architecture, cognitive theory, attack benchmark" in _normalized(orientation)
    assert "MCP references use the 2025-06-18 specification" in _normalized(method_ref)
    assert "external-memory governance, tool boundaries, logging, and retention" in _normalized(method_ref)
    assert "not cognitive-taxonomy claims" in _normalized(method_ref)


def test_sre_circuit_breaker_is_author_defined_not_vendor_attributed(built_output: Path) -> None:
    output_manuscript = manuscript_dir(built_output)
    security_overview = (output_manuscript / "parts" / "ageint-agentic-intelligence" / "ageint-security-and-adversarial-considerations" / "00-overview.md").read_text(encoding="utf-8")
