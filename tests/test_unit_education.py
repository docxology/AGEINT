...
def test_unit_profiles_render_in_unit_introductions(built_output: Path) -> None:
    ...
    for part in curriculum.parts:
        ...
        assert f"## {part['title']} learning spine and source route: unit purpose, module order, and evidence handoff" in text
        assert f"### {part['title']} visual navigation and module map: evidence flow, order, and safety cues" in text
        assert f"### {part['title']} module roster and source-lane inventory: citations, lanes, and learner route" in text
        assert f"### {profile.concept} discipline spine: domain question and learning focus" in text
        assert f"### {profile.concept} source-use contract: citation roles and evidence limits" in text
        assert f"### {profile.concept} practice artifact: recurring packet and retained evidence" in text
        assert f"### {profile.concept} safety boundary: accountable, synthetic, and evidence-bounded limits" in text
...
def test_topic_lessons_include_unit_specific_terms(built_output: Path) -> None:
    ...
    for part in curriculum.parts:
        ...
        for chapter in part["chapters"]:
            lesson_path = root / "parts" / slug_for_path(part["title"]) / slug_for_path(chapter["title"]) / "01-practice-studio.md"
