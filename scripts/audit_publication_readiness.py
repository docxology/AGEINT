...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--write", action="store_true", help="Write output/reports/publication_readiness.{json,md}.")
    parser.add_argument("--skip-parent-guard", action="store_true", help="Do not run the parent template tracked-project confidentiality guard.")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write:
        json_path, md_path, report = write_publication_readiness(PROJECT_ROOT, run_parent_guard=not args.skip_parent_guard)
    else:
        json_path = md_path = None
        report = collect_publication_readiness(PROJECT_ROOT, run_parent_guard=not args.skip_parent_guard)
    if args.format == "json":
        import json
